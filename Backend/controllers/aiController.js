...
const processCodeChat = async (req, res) => {
  let isClientDisconnected = false;
  const abortController = new AbortController();
  ...
  try {
    ...
    const systemPrompt = prompts.getCodeSystemPrompt();
    ...
    if (error.name === 'AbortError') return;
    console.error('Production AI Code Chat Error:', error);
