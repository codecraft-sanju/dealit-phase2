...
import { 
  ...
  Palette, BarChart3, Activity, RefreshCw, X, Eye, AlignCenter, AlignLeft, AlignRight, LayoutPanelTop
} from 'lucide-react';
...
const CreateAIPage = ({ user }) => {
  ...
  // NEW: State for Mobile Bottom Sheet
  ...
  const renderTabContent = () => (
    <AnimatePresence mode="wait">
      ...
      {activeTab === 'wizard' && !hasAgent && (
        <motion.div key="wizard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="max-w-2xl mx-auto w-full flex flex-col h-full justify-center py-6">
          <div className="text-center mb-10 mt-6 lg:mt-0">
            <div className="inline-flex items-center justify-center w-20 h-20 rounded-full mb-4 shadow-[0_0_30px_rgba(168,85,247,0.3)] overflow-hidden bg-white/5 border border-purple-500/20">
          ...
          <div className="flex items-center justify-center gap-3 mb-10 max-w-xs mx-auto">
            {[1, 2].map((s) => (
              <React.Fragment key={s}>
                ...
                {s < 2 && <div className={`flex-1 h-[2px] rounded-full ${step > s ? 'bg-emerald-500/40' : 'bg-white/10'}`} />}
              </React.Fragment>
            ))}
          </div>
            ...
            {step === 1 && (
              <motion.form key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={handleInitAI} className="space-y-6">
                <div className="space-y-1">
                  ...
                  <textarea value={baseIdea} maxLength={300} onChange={(e) => setBaseIdea(e.target.value)} placeholder="e.g., An AI assistant for my freelance web design portfolio that can quote prices..." className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 outline-none h-32 text-sm resize-none transition-all placeholder:text-gray-600" />
                </div>
                <div className="space-y-1">
                  ...
                  <div className="flex items-center bg-white/5 border border-white/10 rounded-2xl overflow-hidden focus-within:ring-2 focus-within:ring-purple-500/50 focus-within:border-purple-500/50 transition-all">
                ...
                <button type="submit" disabled={isLoading} className="w-full bg-white text-black font-bold py-4 rounded-2xl hover:bg-gray-200 active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm disabled:opacity-50 mt-4 shadow-lg">
            ...
            {step === 2 && (
              <motion.form key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} onSubmit={handleSubmitAnswers} className="space-y-6 flex flex-col h-full">
                <div className="flex-1 overflow-y-auto pr-2 space-y-4 no-scrollbar pb-10">
                  {questions.map((q, idx) => (
                    <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-colors">
                      <label className="block text-sm font-medium text-purple-300 mb-3 leading-relaxed">{idx + 1}. {q}</label>
                      <textarea value={answers[idx]} onChange={(e) => { const n = [...answers]; n[idx] = e.target.value; setAnswers(n); }} placeholder="Write your detailed answer here..." className="w-full bg-black/40 border border-white/10 rounded-xl p-4 text-white text-sm focus:ring-2 focus:ring-purple-500/50 outline-none resize-none min-h-[100px] placeholder:text-gray-600 transition-all" />
                    </div>
                  ))}
                </div>
                <button type="submit" disabled={isLoading} className="w-full shrink-0 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 text-sm disabled:opacity-50 hover:opacity-90 active:scale-[0.98] transition-all shadow-[0_0_20px_rgba(16,185,129,0.3)]">
        ...
        <motion.div key="knowledge" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6 lg:space-y-8 max-w-2xl mx-auto lg:mx-0">
          ...
          <form onSubmit={handlePdfUpload} className="space-y-6">
            <div 
              ...
              className={`relative group border-2 border-dashed rounded-3xl p-10 lg:p-12 text-center cursor-pointer transition-all overflow-hidden ${pdfFile ? 'border-purple-500 bg-purple-500/5' : 'border-white/10 hover:border-purple-500/50 bg-white/5 hover:bg-purple-500/5'}`}
            >
              <input type="file" ref={fileInputRef} onChange={(e) => setPdfFile(e.target.files[0])} accept="application/pdf" className="hidden" />
              
              <div className={`w-16 h-16 mx-auto rounded-full flex items-center justify-center mb-4 transition-colors ${pdfFile ? 'bg-purple-500/20 text-purple-400' : 'bg-white/10 text-gray-400 group-hover:bg-purple-500/20 group-hover:text-purple-400'}`}>
            ...
            <div className="flex justify-end">
              <button type="submit" disabled={!pdfFile || isLoading} className="w-full lg:w-auto px-8 bg-white hover:bg-gray-200 text-black font-bold py-4 lg:py-3.5 rounded-2xl lg:rounded-xl transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-sm disabled:opacity-50">
      ...
      {/* --- TAB: APPEARANCE (THEMES & LAYOUTS) --- */}
      {activeTab === 'theme' && hasAgent && (
        <motion.div key="theme" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-8 pb-8 lg:pb-0">
          <div>
            <h2 className="text-2xl font-bold text-white mb-2 tracking-tight">Appearance</h2>
            <p className="text-gray-400 text-sm">Customize how your AI agent looks to visitors.</p>
          </div>
          
          {/* Theme Selection */}
          <div>
            <h3 className="text-sm font-semibold text-gray-300 mb-4 flex items-center gap-2"><Palette className="w-4 h-4"/> Color Theme</h3>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-6">
              {THEMES.map(t => (
                <div 
                  key={t.id} 
                  onClick={() => handleUpdateTheme(t.id)}
                  className={`relative p-1.5 rounded-3xl cursor-pointer transition-all ${currentTheme === t.id ? 'bg-gradient-to-br from-purple-500 to-blue-500 scale-[1.02] shadow-[0_0_30px_rgba(168,85,247,0.3)]' : 'bg-white/5 hover:bg-white/10 active:scale-95'}`}
                >
                  ...
                  {currentTheme === t.id && (
                    <div className="absolute -top-2 -right-2 bg-[#050505] rounded-full p-0.5 border border-white/10">
              ...
              {LAYOUTS.map(l => (
                <div 
                  ...
                  className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 cursor-pointer transition-all ${
                    currentLayout === l.id ? 'border-purple-500 bg-purple-500/10 text-purple-300' : 'border-white/10 bg-white/5 hover:bg-white/10 text-gray-400'
                  }`}
                >
                  <l.icon className={`w-6 h-6 ${currentLayout === l.id ? 'text-purple-400' : 'text-gray-500'}`} />
      ...
      {activeTab === 'analytics' && hasAgent && (
        <motion.div key="analytics" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6 lg:space-y-8 flex flex-col h-full relative">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4 shrink-0">
            <div className="bg-white/5 border border-white/5 rounded-3xl p-5 lg:p-6 flex flex-col justify-between">
            ...
            <div className="col-span-2 lg:col-span-1 bg-white/5 border border-white/5 rounded-3xl p-5 lg:p-6 flex lg:flex-col justify-between items-center lg:items-start">
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#050505] text-white flex flex-col relative font-sans overflow-hidden">
      ...
      <div className="lg:hidden flex flex-col h-[100dvh] bg-[#050505] relative z-10 overflow-hidden">
        ...
        {/* Live Preview Area (Edge-to-Edge like Linktree) */}
        {hasAgent ? (
          <div className="flex-1 relative overflow-hidden bg-[#050505]">
            <iframe 
              key={previewRefreshKey}
              src={`/ai/${myAgentData.username}?preview=true`} 
        ...
        {/* Floating Customize Button (Visible when Bottom Sheet is Closed) */}
        <AnimatePresence>
          {hasAgent && !isMobileSheetOpen && (
            <motion.button
              ...
              className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-white text-black px-6 py-3.5 rounded-full text-sm font-bold shadow-[0_10px_40px_rgba(0,0,0,0.6)] active:scale-95 transition-transform"
            >
        ...
        {/* Bottom Editor Sheet (Draggable & Collapsible) */}
        <AnimatePresence>
          {hasAgent && isMobileSheetOpen && (
            <motion.div 
              initial={{ y: "100%" }}
            ...
            >
              ...
              {/* Linktree style horizontal tabs */}
              ...
              {/* Tab Content with no scrollbars */}
              {/* Added 'touch-pan-y' and pointer-events logic to ensure smooth scrolling inside dragged sheet */}
              <div 
                className="flex-1 overflow-y-auto p-6 no-scrollbar pb-12"
                onPointerDown={(e) => e.stopPropagation()} // Prevents internal scrolling from dragging the whole sheet accidentally
              >
                 {renderTabContent()}
              </div>
            </motion.div>
      ...
      <div className="hidden lg:flex flex-1 w-full max-w-[1600px] mx-auto overflow-hidden relative z-10 py-6 px-8 gap-8">
            ...
            <div className="flex flex-col gap-2 w-full flex-1 overflow-y-auto no-scrollbar">
              {TABS.map(tab => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`w-full flex items-center gap-3 py-3.5 px-4 rounded-2xl text-sm font-semibold transition-all group ${
                    activeTab === tab.id 
        ...
        {hasAgent && (
           <div className="w-[320px] xl:w-[360px] flex flex-col items-center justify-center shrink-0 h-full py-4">
             <div className="w-full flex justify-between items-center mb-4 px-1 shrink-0">
               ...
               <button 
                 ...
                 className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider bg-white/5 hover:bg-white/10 px-3 py-1.5 rounded-full transition-colors text-gray-400 hover:text-white border border-white/10"
               >
             ...
             <div className="w-full flex-1 max-h-[750px] min-h-[500px] rounded-[2.5rem] border-[10px] border-[#18181b] bg-[#050505] overflow-hidden shadow-2xl relative">
