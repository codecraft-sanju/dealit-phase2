...
// CHANGED: Added ThumbsUp and ThumbsDown, removed CheckCheck
import {
  ArrowLeft,
  Send,
  Sparkles,
...
// CHANGED: Added CopyCodeButton component for code blocks
    ...
    >
      <ReactMarkdown
        ...
        components={{
          ...
          h1: ({ node, ...props }) => (
            <h1 className="text-xl font-bold mb-2 mt-4 text-white" {...props} />
          ),
          ...
          code: ({ node, inline, className, children, ...props }) => {
            ...
            if (!inline && isJsonBlock) {
              try {
                ...
                if (
                  parsedData.ui_type === 'product_carousel' &&
                ...
                if (
                  parsedData.ui_type === 'action_button' &&
                  parsedData.label &&
                  parsedData.action
                ) {
                  return (
            ...
            return inline ? (
              ...
              // CHANGED: Added relative wrapper, CopyCodeButton component, and top padding (pt-12) to ensure the button doesn't cover code text
          ...
          th: ({ node, ...props }) => (
            <th
              className="px-4 py-3 text-left font-semibold text-gray-300 uppercase tracking-wider text-xs"
              {...props}
            />
          ),
...
const AiChatPage = ({ user }) => {
  ...
  useEffect(() => {
    if (currentSessionId && messages.length > 0) {
      localStorage.setItem(
        `dealit_ai_history_${currentSessionId}`,
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener('NATIVE_APP_EVENT', handleNativeAppEvent);
  }, []);
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      const cachedHistory = localStorage.getItem(
        `dealit_ai_history_${routeSessionId}`,
      );
      ...
      try {
        ...
        const res = await axios.get(
          `${API_URL}/ai/chat/history/${routeSessionId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
  ...
  const speakText = async (text) => {
    ...
    const currentVoicePref =
      typeof voicePref !== 'undefined'
        ? voicePref
        : localStorage.getItem('dealit_ai_voice_pref') || 'female';
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          errorData.errorCode === 'DAILY_VOICE_LIMIT_REACHED' ||
          response.status === 429
        ) {
  ...
  const processVoiceMessage = async (userMessage) => {
    ...
    try {
      ...
      const isSmartContextEnabled =
        smartContextStr !== null ? JSON.parse(smartContextStr) : true;
  ...
  const handleMicClick = () => {
    ...
    recognition.onerror = (e) => {
      ...
      if (e.error === 'not-allowed') {
        alert(
          "⚠️ Microphone blocked! URL bar ke left icon par click karein aur mic ko 'Allow' karein.",
        );
  ...
  const cancelVoiceMode = () => {
    ...
    if (window.ReactNativeWebView) {
      window.ReactNativeWebView.postMessage(
        JSON.stringify({ type: 'STOP_NATIVE_SPEECH' }),
      );
    }
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2 no-scrollbar">
          <div className="text-xs font-bold tracking-wider text-gray-500 mb-3 px-2 uppercase">
            Recent Chats
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative bg-[#1A1A1A]">
        ...
        <div className="relative flex-1 flex flex-col overflow-hidden">
          ...
          <div
            className={`flex-1 overflow-y-auto p-4 space-y-6 container mx-auto max-w-3xl no-scrollbar relative ${!hasStartedChat && 'flex flex-col items-center justify-center'}`}
          >
            {isLoading && messages.length === 0 ? (
              ...
              >
                ...
                >
                  Your personal assistant for trades, credits, and navigating
                  the Dealit marketplace. How can I help you today?
                </motion.p>
              ...
              messages.map((msg) => (
                ...
                >
                  ...
                  >
                    <div
                      className={`rounded-2xl px-5 py-3.5 text-sm overflow-hidden flex flex-col ${msg.role === 'user' ? 'bg-gradient-to-br from-purple-600 to-purple-500 text-white rounded-tr-sm shadow-lg shadow-purple-500/20' : 'bg-gray-800 text-gray-200 border border-gray-700 rounded-tl-sm shadow-md'}`}
                    >
                      <div
                      ...
                      {/* CHANGED: Removed the footer entirely for user messages and added ThumbsUp/ThumbsDown buttons for AI responses */}
              ...
              >
                ...
                <div className="z-10 flex flex-col items-center w-full max-w-md px-6 text-center">
                  ...
                  <h2 className="text-2xl font-bold text-white mb-3 drop-shadow-md">
                    ...
                    {voiceState === 'speaking' &&
                      (isPremiumVoiceLimited
                        ? 'Speaking (Standard Voice)...'
                        : 'Speaking...')}
                  </h2>
                  ...
                  <div className="h-12 flex items-center justify-center w-full mb-10 text-white drop-shadow-md">
                    ...
                    {(voiceState === 'thinking' ||
                      voiceState === 'generating_audio') && <TypingLoader />}
