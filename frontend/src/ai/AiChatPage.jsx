...
// import React, { useState, useRef, useEffect, useCallback, useMemo } from 'react';
// import { ArrowLeft, Send, Sparkles, Menu, Plus, Settings, HelpCircle, MessageSquare, X, Trash2, Minimize2, ChevronDown, Mic, User, WifiOff, Copy, Check, CheckCheck, Home, Search, Bot, Headphones, ChevronRight, Zap, Shield, Star } from 'lucide-react';
// import { useNavigate, useParams } from 'react-router-dom';
// import { motion, AnimatePresence } from 'framer-motion'; 
// import axios from 'axios';
// import ReactMarkdown from 'react-markdown';
...
// // FAQ data for the home state search
// const FAQ_ITEMS = [
//   { id: 1, question: "Check Aura Score", icon: <Star className="w-4 h-4 text-yellow-400" />, prompt: "What is my Aura Score and how is it calculated?" },
//   { id: 2, question: "Marketplace Rules", icon: <Shield className="w-4 h-4 text-emerald-400" />, prompt: "What are the Dealit marketplace rules and guidelines?" },
//   { id: 3, question: "How Credits Work", icon: <Zap className="w-4 h-4 text-purple-400" />, prompt: "How do Dealit credits work and how can I earn more?" },
//   { id: 4, question: "Delivery Verification", icon: <CheckCheck className="w-4 h-4 text-blue-400" />, prompt: "Explain OTP delivery verification process" },
// ];

// // Home state panel shown before any chat starts — mirrors MongoDB's widget home
// const WidgetHomePanel = ({ user, onStartChat, onQuickLink, onNavigate }) => {
//   const [searchQuery, setSearchQuery] = useState('');
//   const isOperational = true; // Could be fetched from a status API
...
//       className="flex flex-col items-center justify-start w-full max-w-lg mx-auto pt-2 pb-4 px-1"
//     >
//       {/* Greeting */}
...
//       {/* System status card */}
//       <div className="w-full bg-gray-800/60 border border-gray-700/50 rounded-2xl p-4 mb-3 flex items-center gap-3">
...
//       {/* Search + quick links */}
//       <div className="w-full bg-gray-800/40 border border-gray-700/50 rounded-2xl overflow-hidden mb-3">
//         {/* Dummy search bar */}
...
//             <button
//               key={item.id}
//               onClick={() => onQuickLink(item.prompt)}
//               className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-gray-700/30 transition-colors text-left group"
//             >
//               <div className="flex items-center gap-3">
//                 <span className="flex-shrink-0">{item.icon}</span>
//                 <span className="text-sm text-gray-200 font-medium">{item.question}</span>
//               </div>
...
//               <button onClick={() => onQuickLink(`${searchQuery}`)} className="text-xs text-purple-400 mt-1 hover:text-purple-300 transition-colors">
//                 Ask AI about this →
//               </button>
//             </div>
//           )}
...
//       {/* Ask Dealit AI card — bot + human overlapping icons */}
//       <button
//         onClick={onStartChat}
//         className="w-full bg-gray-800/40 border border-purple-500/30 hover:border-purple-500/60 hover:bg-purple-500/10 rounded-2xl p-4 flex items-center justify-between transition-all group"
//       >
//         <div className="flex flex-col items-start gap-0.5">
//           <span className="text-sm font-bold text-white">Ask Dealit AI or Support Team</span>
//           <span className="text-xs text-gray-400">AI Agent and team can help</span>
//         </div>
//         {/* Overlapping bot + human avatars */}
//         <div className="flex items-center flex-shrink-0 ml-3">
//           <div className="w-8 h-8 rounded-full bg-purple-600/80 border-2 border-gray-800 flex items-center justify-center z-10">
//             <Bot className="w-4 h-4 text-white" />
//           </div>
//           <div className="w-8 h-8 rounded-full bg-emerald-600/80 border-2 border-gray-800 flex items-center justify-center -ml-2 z-20">
//             <Headphones className="w-4 h-4 text-white" />
//           </div>
//           <div className="w-8 h-8 rounded-full bg-blue-600/80 border-2 border-gray-800 flex items-center justify-center -ml-2 z-30">
...
//   >
//     <h3 className="text-base font-bold text-white mb-1">Help & Resources</h3>
//     {[
//       { label: 'Marketplace Rules & Guidelines', icon: <Shield className="w-4 h-4 text-emerald-400" /> },
//       { label: 'How Credits & Aura Score Work', icon: <Star className="w-4 h-4 text-yellow-400" /> },
//       { label: 'Delivery & Verification Help', icon: <CheckCheck className="w-4 h-4 text-blue-400" /> },
//       { label: 'Report an Issue', icon: <Zap className="w-4 h-4 text-red-400" /> },
//     ].map((item, i) => (
//       <button
//         key={i}
//         onClick={() => onNavigate('/help-support')}
//         className="flex items-center justify-between w-full p-3.5 rounded-xl bg-gray-800/50 border border-gray-700/40 hover:bg-gray-700/40 transition-colors group"
//       >
//         <div className="flex items-center gap-3">
//           {item.icon}
//           <span className="text-sm text-gray-200 font-medium text-left">{item.label}</span>
//         </div>
...
//   const [isLoading, setIsLoading] = useState(false); 
//   const [hasStartedChat, setHasStartedChat] = useState(false);
//   // activeTab drives the bottom nav: 'home' | 'messages' | 'help'
...
//   // Handle bottom nav tab switches
//   const handleTabChange = (tab) => {
//     setActiveTab(tab);
//     if (tab === 'messages' && !hasStartedChat) {
//       // Don't switch to messages view unless a chat has started; keep on home
//       setActiveTab('home');
//     }
//   };
...
//   // Derives what the main content area should show based on activeTab + chat state
//   const renderMainContent = () => {
//     if (activeTab === 'help') {
//       return (
//         <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-purple-500/20 scrollbar-track-transparent">
//           <HelpPanel onNavigate={navigate} />
//         </div>
//       );
//     }

//     if (activeTab === 'home' && !hasStartedChat) {
//       return (
//         <div className="flex-1 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-purple-500/20 scrollbar-track-transparent">
//           <WidgetHomePanel
//             user={user}
//             onStartChat={() => {
//               // Opens the chat input area without sending anything yet
//               setHasStartedChat(true);
//               setActiveTab('messages');
//             }}
//             onQuickLink={(prompt) => processMessage(prompt)}
//             onNavigate={navigate}
//           />
//         </div>
//       );
...
//     // messages tab or after chat has started
//     return (
//       <>
//         <div className={`flex-1 overflow-y-auto p-4 space-y-6 container mx-auto max-w-3xl scrollbar-thin scrollbar-thumb-purple-500/20 scrollbar-track-transparent relative ${messages.length === 0 && 'flex flex-col items-center justify-center'}`}>
//           {isLoading && messages.length === 0 ? (
//             <GeneratingLoader />
//           ) : messages.length === 0 ? (
//             // Blank slate after "Ask Dealit AI" card is tapped but before first message
//             <motion.div
//               initial={{ opacity: 0, y: 20 }}
//               animate={{ opacity: 1, y: 0 }}
//               transition={{ duration: 0.8, ease: 'easeOut' }}
...
//         {/* Chat input — only shown on messages tab */}
//         <div className="shrink-0 pb-safe z-10 relative">
//           <div className="p-4 container mx-auto max-w-3xl">
//             <form onSubmit={handleSendMessage} className="flex items-center gap-2">
//               <div className="relative flex-1 flex items-center">
...
//             <motion.div
//               key={activeTab === 'help' ? 'help' : activeTab === 'home' && !hasStartedChat ? 'home' : 'chat'}
//               initial={{ opacity: 0 }}
//               animate={{ opacity: 1 }}
//               exit={{ opacity: 0 }}
