  ...
  return (
    <>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-6 py-3 pb-safe z-50 flex justify-between items-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        ...
        <Link to="/wallet" className={`flex flex-col items-center gap-1 ${location.pathname === '/wallet' ? 'text-yellow-500' : 'text-gray-400'}`}>
          <Coins className="w-6 h-6" />
          <span className="text-[10px] font-medium">Earn</span>
        </Link>

        {/* Floating Add Button */}
        <Link to="/add-item" className="relative -top-5 bg-[#A388E1] text-white p-3 rounded-full shadow-lg shadow-[#A388E1]/40 border-4 border-white">
          <PlusCircle className="w-6 h-6" />
        </Link>

        <Link to="/messages" className={`flex flex-col items-center gap-1 ${location.pathname === '/messages' ? 'text-[#A388E1]' : 'text-gray-400'}`}>
          <MessageSquare className="w-6 h-6" />
          <span className="text-[10px] font-medium">Chat</span>
        </Link>

        <Link to="/profile" className={`flex flex-col items-center gap-1 ${location.pathname === '/profile' ? 'text-[#A388E1]' : 'text-gray-400'}`}>
